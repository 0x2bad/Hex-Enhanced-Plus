...
functions.university.getResearchVariables = function(){
    ...
    if (r.status == 200){
        ...
        var money = (json.finances).match(/\d/g).join("");
    }
    var cost = $(".widget-box.text-left").find(".green").text().match(/\d/g).join("");
    var time = $(":contains('minutes'):last").text().match(/\d/g).join("");
};
